...
public class Human_fileController {
	...
	public Human_fileController() {
		super();
		System.out.println("Human_fileController");
	}
	
	
	@RequestMapping(value ="toHR-specialist")
	public String toHR_specialist(){
		logger.info("toHR-specialist called....");
		return "HR_specialist";
	}
	//进入人力资源档案登记页面HR_file_reg
	/*
	@RequestMapping(value = "toHR_file_reg")
	public String listHR_file_reg(Model model) { // model 由
		logger.info("HR_file_reg called");
		return "HR_file_reg";

	}*/
	/*
	@RequestMapping(value = "tofileRegSubmit")
	public  String fileRegSubmit(Model model) { //  model 由
		logger.info("fileRegSubmit called...");
		return "success";
	}
	*/
	@RequestMapping(value = "toHR_file_reg")
	public String toHR_file_reg(Model model){
		...
		List<Config_public_char> config_public_chars = this.human_fileBiz.getAllConfig_public_chars();     //  this.bookBiz.getAllCategories();
